...
export function cmdLog({
  ...
  if (msgObj) {
    ...
    if (actualMsg.imageMessage) {
      ...
      if (isViewOnce) {
        ...
        tipoMedio = actualMsg.audioMessage.ptt
          ? "AUDIO (Nota de Voz) 🎙️"
          : "AUDIO (Archivo) 🎵";
      }
    }
  ...
  const tipoAccion = commandName
    ? chalk.cyan.bold(" COMANDO ")
    : chalk.green.bold(" MENSAJE ");
  ...
  if (commandName) {
    ...
    contenido =
      chalk.magenta.bold(`MEDIO: ${tipoMedio} `) +
      (caption ? chalk.white(` "${caption.substring(0, 20)}..."`) : "");
  } else {
    contenido = chalk.white(
      `"${text?.substring(0, 35)}${text?.length > 40 ? "..." : ""}"`,
    );
  }
