...
export default {
    ...
    execute: async (socket, message, args, { db, saveDB }) => {
        ...
        const protectedCategories = ['owner', 'group', 'system'];
        const validCategories = ['fun', 'utils', 'downloads', 'search', 'economy', 'sticker', 'profile', 'interaction', 'AI'];
        ...
        if (action === 'on' || action === '1' || action === 'true' || action === 'activar' || action === 'enable') {
        ...
        } else if (action === 'off' || action === '0' || action === 'false' || action === 'desactivar' || action === 'disable') {
