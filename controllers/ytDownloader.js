...
class YTDownloader {
  ...
  async raceApis(url, type = "audio") {
    const apis = [
      {
        ...
        fn: async () => {
          const endpoint =
            type === "audio"
              ? "api/v1/descargas/youtube"
              : "api/v1/descargas/youtubev2";
          const typeParam = type === "audio" ? "mp3" : "video";
          const qualityParam = type === "audio" ? "" : "&quality=720";
          const res = await axios.get(
            `${global.Apis.apiCausa.url}${endpoint}?apikey=${global.Apis.apiCausa.apikey}&url=${encodeURIComponent(url)}&type=${typeParam}${qualityParam}`,
            { timeout: 60000 },
          );
      ...
      },
      {
        ...
        fn: async () => {
          ...
          const res = await axios.get(
            `${global.Apis.appiFaa.url}${endpoint}?url=${encodeURIComponent(url)}`,
            { timeout: 20000 },
          );
          return (
      ...
      },
      {
        ...
        fn: async () => {
          const endpoint =
            type === "audio" ? "download/ytmp3" : "download/ytmp4";
          const res = await axios.get(
            `${global.Apis.deliriusApi.url}${endpoint}?url=${encodeURIComponent(url)}`,
            { timeout: 20000 },
          );
          return (
            res.data?.data?.download || res.data?.result?.url || res.data?.url
          );
        },
      },
    ];
    ...
    try {
        ...
        apis.map((api) =>
          api.fn().then((resUrl) => {
            if (!resUrl || typeof resUrl !== "string")
              throw new Error(`[${api.name}] No devolvió URL válida`);
      ...
      console.error(
        "[Carrera] Todas las APIs fallaron:",
        e.errors || e.message,
      );
      throw new Error(
        "Todas las APIs de descarga están saturadas o caídas. Intenta más tarde.",
      );
    }
  }
