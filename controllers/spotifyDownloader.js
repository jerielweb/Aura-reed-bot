...
async function downloadFile(jobId) {
  ...
  if (buffer.length > MAX_BYTES) throw new Error(`Archivo demasiado grande: ${formatSize(buffer.length)}`)
  if (buffer.length < 1024) throw new Error(`Archivo inválido (${buffer.length} bytes)`)

  const ct = String(res.headers.get('content-type') || '')
  const esId3 = buffer[0] === 0x49 && buffer[1] === 0x44 && buffer[2] === 0x33
  const esFrame = buffer[0] === 0xff && ((buffer[1] ?? 0) & 0xe0) === 0xe0

  if (!esId3 && !esFrame && !ct.includes('audio') && !ct.includes('octet-stream')) {
    throw new Error(`No parece un MP3 válido (content-type: ${ct || 'desconocido'})`)
  }
