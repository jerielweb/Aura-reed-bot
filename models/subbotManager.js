  ...
  try {
    ...
    if (Array.isArray(subbots)) {
      return subbots
        ...
        .map((id) => ({
          id,
          active: activeSubBots.has(id),
        }));
    }

    if (
      subbots &&
      typeof subbots === "object"
    ) {
      return Object.keys(subbots)
        .map((id) =>
          String(id).replace(/\D/g, ""),
        )
        .filter(Boolean)
    ...
    console.error(
      "[SUB-BOT] Error leyendo subbots.json:",
      error,
    );
...
export async function getSubBotsInGroup(groupJid) {
  ...
  for (
    const [id, subSock] of activeSubBots
  ) {
    try {
      if (
        !subSock ||
      ...
      // El Sub-Bot no está en este grupo.
    }
  }
