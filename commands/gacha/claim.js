...
export default {
  ...
  execute: async (socket, message, args, extra) => {
    ...
    if (!quotedId) {
      await ctx.reply(box("🎴", "RECLAMAR", "❌ Falta la respuesta...", [], "↩️ Responde a la imagen del personaje que quieres reclamar."));
      return;
    }
    ...
    if (!claimKey) {
      await ctx.reply(box("🎴", "RECLAMAR", "⏰ No disponible...", [], "Este personaje ya no está disponible o expiró."));
      return;
    }
    ...
    if (!pending || Date.now() > pending.expiresAt) {
      ...
      await ctx.reply(box("🎴", "RECLAMAR", "⏰ Tiempo agotado...", [], "El tiempo para reclamar expiró."));
      return;
    }

    if (pending.roller && pending.roller !== ctx.sender) {
      await ctx.reply(box("🔒", "RECLAMAR", "🚫 No te pertenece...", [], "Solo quien usó *.rw* puede reclamar a este personaje."));
      return;
    }
    ...
    try {
      ...
      await ctx.reply(box("✅", "PERSONAJE RECLAMADO", `💞 ${char.name}`, [
        `⚥GÉNERO › ${char.gender}`,
        `📖SERIE › ${char.series}`,
      ...
      if (e.message === "CHARACTER_ALREADY_OWNED") {
        await ctx.reply(box("⚠️", "RECLAMAR", "Ya lo tienes...", [], `Ya tienes a *${char.name}* en tu harem.`));
        return;
      }
      await ctx.reply(box("❌", "RECLAMAR", "Error...", [], `Error: ${e.message}`));
    }
  },
};
