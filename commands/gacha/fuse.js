...
export default {
  ...
  category: "gacha",
  description: "🔀 Fusiona 2 personajes para crear uno más fuerte. .fuse <nº1> <nº2>",
  execute: async (socket, message, args, extra) => {
    ...
    if (harem.length < 2) {
      await ctx.reply(box("🔀", "FUSIÓN", "Necesitas al menos 2 personajes...", [], "Consigue más personajes con *.rw* o *.dailyrw*"));
      return;
    }
    ...
    if (isNaN(num1) || isNaN(num2) || num1 < 1 || num2 < 1 || num1 > harem.length || num2 > harem.length) {
      await ctx.reply(box("🔀", "FUSIÓN", "Uso:", [
    ...
    if (num1 === num2) {
      await ctx.reply(box("🔀", "FUSIÓN", "No puedes fusionar un personaje consigo mismo...", [], "Elige 2 diferentes."));
      return;
    }
    ...
    if (fav && (char1.id === fav.id || char2.id === fav.id)) {
      await ctx.reply(box("⚠️", "FUSIÓN", "No puedes fusionar a tu favorito... 💔", [], "Usa *.fav remove* primero."));
      return;
    }
    ...
    try {
      ...
      await ctx.reply(box("🔀", "⚡ FUSIÓN EXITOSA ⚡", result.name, [
        `📖 SERIE › ${result.series}`,
