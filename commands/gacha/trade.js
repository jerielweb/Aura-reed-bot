...
export default {
  ...
  category: "gacha",
  description: "🎁 Regala un personaje a otro usuario. .trade <@usuario> <número>",
  execute: async (socket, message, args, extra) => {
    ...
    if (!targetNum) {
      ...
      if (quotedParticipant) {
        targetNum = quotedParticipant.split("@")[0]?.replace(/[^0-9]/g, "") || "";
      }
    }

    if (!targetNum) {
      await ctx.reply(box("🎁", "TRADE", "Uso:", [
    ...
    if (targetJid === userId) {
      await ctx.reply(box("🎁", "TRADE", "No puedes regalarte a ti mismo...", [], "Usa *.dump* si quieres eliminar un personaje por dinero."));
      return;
    }

    const num = parseInt(cmdArgs[1]);
    if (isNaN(num)) {
      await ctx.reply(box("🎁", "TRADE", "Falta el número...", [], "Usa: *.trade @usuario <número>* — El número lo ves en *.harem*"));
      return;
    }

    const harem = gacha.getUserHarem(userId);
    if (harem.length === 0) {
      await ctx.reply(box("🎁", "TRADE", "Tu harem está vacío...", [], "No tienes personajes para regalar."));
      return;
    }

    if (num < 1 || num > harem.length) {
      await ctx.reply(box("🎁", "TRADE", "Número inválido...", [], `Tienes ${harem.length} personajes. Usa *.harem* para ver los números.`));
      return;
    }
    ...
    try {
      ...
      await ctx.reply(box("✅", "TRADE COMPLETADO", `✨ *${char.name}* ha sido transferido(a)!`, [
        `📖SERIE › ${char.series}`,
        `💴VALOR › ${char.value.toLocaleString()} ¥`,
        `👤NUEVO DUEÑO › @${targetNum}`,
      ], `💡 El personaje ahora pertenece a @${targetNum}.`), [targetJid]);
    } catch (e) {
      await ctx.reply(box("❌", "TRADE", "Error...", [], `Error: ${e.message}`));
    }
  },
};
