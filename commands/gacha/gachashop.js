...
export default {
  ...
  category: "gacha",
  description:
    "🏪 Mercado de personajes entre usuarios. .gshop list <nº> <precio> | .gshop buy <nº> | .gshop",
    ...
    if (!sub) {
      ...
      if (listings.length === 0) {
        await ctx.reply(
          box(
            ...
            [],
            "Usa *.gshop list <nº> <precio>* para vender un personaje.",
          ),
        );
        return;
      }
      ...
      return ctx.reply(
        box(
          "🏪",
          "GACHA SHOP",
          `${listings.length} personaje(s) en venta`,
          fields,
          "Usa *.gshop buy <nº>* para comprar. Usa *.gshop list <nº> <precio>* para vender.",
        ),
      );
    }
    ...
    if (sub === "list" || sub === "vender" || sub === "add") {
      ...
      if (isNaN(num) || num < 1 || num > harem.length) {
        const fields = harem
          ...
          .map(
            (c, i) =>
              `${i + 1}. *${c.name}* — 💴 ${c.value.toLocaleString()} ¥`,
          );
        return ctx.reply(
          box(
            "🏪",
            "GACHA SHOP",
            "Uso: .gshop list <nº> <precio>",
            ["", "📋 Tus personajes:", ...fields],
      ...
      if (isNaN(price) || price < 100) {
        return ctx.reply(
          box(
            ...
            "Precio inválido...",
            [],
            "El precio mínimo es 100 ¥. Ej: *.gshop list 1 5000*",
          ),
        );
      }
      ...
      if (num > listings.length) {
        return ctx.reply(
          box(
            "🏪",
            "GACHA SHOP",
            "Ese listing no existe...",
            [],
            `Hay ${listings.length} personajes en venta.`,
          ),
        );
      }
      ...
      if (isNaN(num) || num < 1) {
        return ctx.reply(
          box(
            ...
            "Uso: .gshop unlist <nº>",
            [],
            "El número corresponde al listing en *.gshop*",
          ),
        );
      }
      ...
      try {
        ...
        return ctx.reply(
          box(
            "✅",
            "GACHA SHOP",
            `*${char.name}* retirado del mercado`,
            [],
            "El personaje volvió a tu harem.",
          ),
        );
      } catch (e) {
        return ctx.reply(box("❌", "GACHA SHOP", "Error...", [], e.message));
