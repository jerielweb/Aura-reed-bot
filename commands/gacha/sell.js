...
export default {
  ...
  execute: async (socket, message, args, extra) => {
    ...
    if (harem.length === 0) {
      await ctx.reply(box("💰", "VENTA", "Tu harem está vacío...", [], "No tienes personajes para vender."));
      return;
    }
    ...
    if (!arg) {
      const fields = harem.slice(0, 10).map((c, i) => `${i + 1}. *${c.name}* — 💴 ${c.value.toLocaleString()} ¥ (venta ~${Math.floor(c.value * 0.6).toLocaleString()} ¥)`);
      if (harem.length > 10) fields.push(`...y ${harem.length - 10} más`);

      await ctx.reply(box("💰", "VENTA", "Uso:", [
    ...
    if (arg === "all" || arg === "todo" || arg === "-a") {
      ...
      if (vendidos === 0) {
        await ctx.reply(box("💰", "VENTA", "No se pudo vender nada...", errors.length ? errors : [], "Revisa tu harem e intenta de nuevo."));
        return;
      }

      ctx.addCoins(totalGanancia);

      const lines = [`💴 Ganancia total: *${totalGanancia.toLocaleString()} ¥*`];
      if (errors.length) lines.push(...errors.map((e) => `⚠️ ${e}`));

      await ctx.reply(box("💰", `VENTA MASIVA COMPLETADA`, `${vendidos} personaje(s) vendido(s)`, lines, `💵 Se agregaron *${totalGanancia.toLocaleString()} ¥* a tu billetera.`));
      return;
    }

    const num = parseInt(arg);
    if (isNaN(num) || num < 1 || num > harem.length) {
      await ctx.reply(box("💰", "VENTA", "Número inválido...", [], `Tienes ${harem.length} personajes. Usa *.harem* para ver los números.`));
      return;
    }
    ...
    if (fav && char.id === fav.id) {
      await ctx.reply(box("⚠️", "VENTA", `No puedes vender a tu favorito... 💔`, [], "Usa *.fav remove* primero si quieres venderlo."));
      return;
    }
    ...
    try {
      ...
      await ctx.reply(box("💰", "VENTA COMPLETADA", `*${char.name}* ha sido vendido(a)`, [
        `📖SERIE › ${char.series}`,
