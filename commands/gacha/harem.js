...
export default {
  ...
  category: "gacha",
  description:
    "👥 Muestra tu colección de personajes. .harem | .harem <nombre>",
  execute: async (socket, message, args, extra) => {
    ...
    const isPageNumber =
      firstArg && !isNaN(pageNum) && pageNum > 0 && ctx.args.length === 1;
    ...
    if (searchQuery && !isPageNumber) {
      ...
      const fields = results
        ...
        .map(
          (c) =>
            `• *${c.name}* — ${c.series} (${c.gender}) — 💴 ${c.value.toLocaleString()} ¥`,
        );
      ...
      await ctx.reply(
        box(
          ...
          fields,
          `📦 ${results.length} personaje(s) encontrado(s)`,
        ),
      );
      return;
    }
    ...
    if (harem.length === 0) {
      await ctx.reply(
        box(
          ...
          "Está vacío... 😔",
          [],
          `Usa *.rw* para conseguir personajes. (${totalChars} disponibles en total)`,
        ),
      );
      return;
    }
    ...
    const fields = pageItems.map(
      (c, i) =>
        `${start + i + 1}. *${c.name}* — ${c.series} (${c.gender}) — 💴 ${c.value.toLocaleString()} ¥`,
    );
    ...
    await ctx.reply(
      box("👥", "MI HAREM", `${ctx.sender.split("@")[0]}`, fields, tip),
    );
  },
};
