...
export default {
    ...
    execute: async (sock, m, args, { prefix, db, saveDB, isOwner, numeroReal }) => {
        ...
        const getMediaMessage = (msg) => {
            ...
            const unwrapped = content?.viewOnceMessage?.message || content?.viewOnceMessageV2?.message || content?.documentWithCaptionMessage?.message || content;
        ...
        const mediaMsg = quotedMsg ? getMediaMessage(quotedMsg) : getMediaMessage(m.message);
        ...
        try {
            ...
            const mimetype = isVideo ? mediaMsg.videoMessage.mimetype : mediaMsg.imageMessage.mimetype;
