...
export default {
  name: ["fap", "masturbar"],
  category: "nsfw",
  description: "Masturbarse pensando en alguien",
  async execute(sock, m, args, { prefix, jidRemitente }) {
    ...
    try {
      const gifData = await getNsfwReactionGif("fap");
      ...
      if (targetJid && targetJid !== jidRemitente) {
        ...
        caption =
          senderTag +
          " " +
          fytBold("se masturba pensando en") +
          " @" +
          targetJid.split("@")[0] +
          " 🔞";
      } else {
        caption = senderTag + " " + fytBold("se está masturbando 🔞");
      }
