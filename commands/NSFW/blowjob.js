...
export default {
  name: ["blowjob", "bj", "mamada", "mamar"],
  category: "nsfw",
  description: "Hacer una mamada a un usuario",
  async execute(sock, m, args, { prefix, jidRemitente }) {
    ...
    try {
      const gifData = await getNsfwReactionGif("blowjob");
      ...
      if (targetJid && targetJid !== jidRemitente) {
        ...
        caption =
          senderTag +
          " " +
          fytBold("le hizo una mamada a") +
          " @" +
          targetJid.split("@")[0] +
          " 🔞";
      } else {
        caption = senderTag + " " + fytBold("quiere mamar 🔞");
      }
