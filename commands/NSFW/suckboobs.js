...
export default {
  name: ["suckboobs", "chupartetas", "mamatetas"],
  category: "nsfw",
  description: "Chupar los pechos a un usuario",
  async execute(sock, m, args, { prefix, jidRemitente }) {
    ...
    try {
      const gifData = await getNsfwReactionGif("suckboobs");
      ...
      if (targetJid && targetJid !== jidRemitente) {
        ...
        caption =
          senderTag +
          " " +
          fytBold("le chupó los pechos a") +
          " @" +
          targetJid.split("@")[0] +
          " 🔞";
      } else {
        caption = senderTag + " " + fytBold("chupa sus propios pechos 🔞");
      }
