...
export default {
  name: ["cummouth", "cumemboca"],
  category: "nsfw",
  description: "Venirse en la boca de un usuario",
  async execute(sock, m, args, { prefix, jidRemitente }) {
    ...
    try {
      const gifData = await getNsfwReactionGif("cummouth");
      ...
      if (targetJid && targetJid !== jidRemitente) {
        ...
        caption =
          senderTag +
          " " +
          fytBold("se vino en la boca de") +
          " @" +
          targetJid.split("@")[0] +
          " 🔞";
      } else {
        caption = senderTag + " " + fytBold("se vino en su propia boca 🔞");
      }
