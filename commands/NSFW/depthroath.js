...
export default {
  name: ["deepthroat", "gargantaprofunda"],
  category: "nsfw",
  description: "Garganta profunda a un usuario",
  async execute(sock, m, args, { prefix, jidRemitente }) {
    ...
    try {
      const gifData = await getNsfwReactionGif("deepthroat");
      ...
      if (targetJid && targetJid !== jidRemitente) {
        ...
        caption =
          senderTag +
          " " +
          fytBold("le hizo garganta profunda a") +
          " @" +
          targetJid.split("@")[0] +
          " 🔞";
      } else {
