...
export default {
  name: ["handjob", "paja", "pajear"],
  category: "nsfw",
  description: "Hacer una paja a un usuario",
  async execute(sock, m, args, { prefix, jidRemitente }) {
    ...
    try {
      const gifData = await getNsfwReactionGif("handjob");
      ...
      if (targetJid && targetJid !== jidRemitente) {
        ...
        caption =
          senderTag +
          " " +
          fytBold("le hizo una paja a") +
          " @" +
          targetJid.split("@")[0] +
          " 🔞";
      } else {
        caption = senderTag + " " + fytBold("se está haciendo una paja 🔞");
      }
