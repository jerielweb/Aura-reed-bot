export default {
    name: ['delowner', 'removeowner', 'rmowner', 'quitarowner', 'quitarpropietario'],
    category: 'owner',
    ...
    execute: async (socket, message, args, { db, saveDB }) => {
        ...
        if (!Array.isArray(db.owners) || db.owners.length === 0) {
            return await socket.sendMessage(remoteJid, { text: '⚠️ No hay owners registrados en la base de datos.' }, { quoted: message });
        }
        ...
        if (db.owners.length === 1) {
            return await socket.sendMessage(remoteJid, { text: '❌ No se puede quitar al último owner del bot.' }, { quoted: message });
        }
        ...
        await socket.sendMessage(remoteJid, { text, mentions: [userToRemove] }, { quoted: message });
    }
};
