...
export default {
    ...
    execute: async (socket, message, args, { db, saveDB, jidRemitente }) => {
        ...
        if (isNaN(amount) || amount <= 0) {
            return await socket.sendMessage(remoteJid, { text: '⚠️ Cantidad inválida. Usa: *.ruleta [cantidad] [color]*\nColores: *red*, *black*, *green*' }, { quoted: message });
        }

        if (!color || !['red', 'black', 'green'].includes(color)) {
            return await socket.sendMessage(remoteJid, { text: '⚠️ Debes elegir un color: *red*, *black* o *green*\nEjemplo: *.ruleta 5000 red*' }, { quoted: message });
        }

        if ((user.coins || 0) < amount) {
            return await socket.sendMessage(remoteJid, { text: `❌ No tienes suficientes monedas. Tienes *₡${user.coins || 0}*` }, { quoted: message });
        }

        const random = Math.random() * 100;
        let result, winnings = 0, resultColor = '';
