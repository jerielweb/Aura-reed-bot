...
export default {
  ...
  execute: async (socket, message, args, { prefix }) => {
    ...
    function unwrapMessage(msg) {
      if (!msg) return null;
      if (msg.stickerMessage || msg.imageMessage || msg.documentMessage)
        return msg;
    ...
    try {
      ...
      if (
        target.stickerMessage ||
        (target.documentMessage &&
          target.documentMessage.mimetype === "image/webp")
      ) {
        const isAnimated =
          target.stickerMessage?.isAnimated ||
          (target.documentMessage &&
            target.documentMessage.mimetype === "image/webp" &&
            target.documentMessage.url?.includes("animated"));
        ...
        target.documentMessage &&
        target.documentMessage.mimetype === "image/gif"
      ) {
        await socket.sendMessage(
          remoteJid,
          {
