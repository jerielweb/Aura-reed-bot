...
export default {
    ...
    execute: async (socket, message, args, { prefix }) => {
        ...
        function unwrapMessage(msg) {
            if (!msg) return null;
            if (msg.imageMessage || msg.videoMessage || msg.documentMessage || msg.audioMessage || msg.stickerMessage) return msg;
        ...
        try {
            ...
            if (!buffer || buffer.length === 0) throw new Error('No se pudo descargar el medio.');
            ...
            if (target.imageMessage) {
                await socket.sendMessage(remoteJid, { image: buffer, caption: `🔁 ${fytBold('Aqui tienes la imagen mi compa')}` }, { quoted: quotedObj });
            } else if (target.videoMessage) {
                await socket.sendMessage(remoteJid, { video: buffer, caption: `🔁 ${fytBold('Aqui tienes el video mi compa')}` }, { quoted: quotedObj });
            }  else {
