...
export default {
    ...
    description: 'Envía una reacción "kabedon" en formato GIF.',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            const { image, mimetype } = await getReactionGif('kabedon');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('le hace kabedon a') + ' ' + targetTag;
            } else {
                caption = senderTag + ' ' + fytBold('hace un kabedon contra la pared 😏');
            }
