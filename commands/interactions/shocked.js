...
export default {
    ...
    description: 'Envía una reacción "shocked" en formato GIF.',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            const { image, mimetype } = await getReactionGif('shocked');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('está sorprendido/a por') + ' ' + targetTag;
            } else {
                caption = senderTag + ' ' + fytBold('está en shock 😲');
            }
