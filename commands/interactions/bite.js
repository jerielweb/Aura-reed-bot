...
export default {
    name: ['bite', 'morder', 'mordió', 'mordida'],
    category: 'interaction',
    description: 'Envía una reacción "bite" en formato GIF.',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            const { image, mimetype } = await getReactionGif('bite');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('mordió a') + ' ' + targetTag;
            } else {
                caption = senderTag + ' ' + fytBold('se está mordiendo 😬');
            }
