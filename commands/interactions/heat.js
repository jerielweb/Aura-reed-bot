...
export default {
    ...
    description: 'Envía una reacción "heat" en formato GIF.',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            const { image, mimetype } = await getReactionGif('heat');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('tiene calor por') + ' ' + targetTag;
            } else {
                caption = senderTag + ' ' + fytBold('tiene mucho calor 🥵');
            }
