...
export default {
    ...
    description: 'Envía una reacción "dramatic" en formato GIF.',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            const { image, mimetype } = await getReactionGif('dramatic');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('es dramático/a con') + ' ' + targetTag;
            } else {
                caption = senderTag + ' ' + fytBold('está siendo muy dramático/a 🎭');
            }
