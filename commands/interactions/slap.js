...
export default {
    ...
    description: 'Envía una reacción "slap" en formato GIF.',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            const { image, mimetype } = await getReactionGif('slap');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('le dio una bofetada a') + ' ' + targetTag;
            } else {
                caption = senderTag + ' ' + fytBold('se dio una bofetada a sí mismo/a 👋');
            }
