...
export default {
    name: ['baka', 'estupido', 'idiota', 'tonto',  'dumb', 'stupid', 'idiot', 'baboso', 'babosa', 'tonta', 'pendejo', 'pendeja', 'pendejo', 'pendeja', 'pendejo', 'pendeja', 'tonto', 'bobo', 'boba', 'estupido', 'baboso', 'babosa', 'estupido', 'estupido', 'estupida'],
    category: 'interaction',
    description: 'Envía una reacción "baka" en formato GIF.',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            const { image, mimetype } = await getReactionGif('baka');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('le dice baka (tonto/a) a') + ' ' + targetTag;
            } else {
