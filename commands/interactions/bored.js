...
export default {
    ...
    description: 'Envía una reacción "bored" en formato GIF.',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            const { image, mimetype } = await getReactionGif('bored');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('se aburre con') + ' ' + targetTag;
            } else {
                caption = senderTag + ' ' + fytBold('está aburrido/a 😑');
            }
