...
export default {
  name: ["creampie", "rellenar"],
  category: "nsfw",
  description: "Hacer un creampie a un usuario",
  async execute(sock, m, args, { prefix, jidRemitente }) {
    ...
    try {
      const gifData = await getNsfwReactionGif("creampie");
      ...
      if (targetJid && targetJid !== jidRemitente) {
        ...
        caption =
          senderTag +
          " " +
          fytBold("le hizo un creampie a") +
          " @" +
          targetJid.split("@")[0] +
          " 🔞";
      } else {
        caption = senderTag + " " + fytBold("quiere un creampie 🔞");
      }
