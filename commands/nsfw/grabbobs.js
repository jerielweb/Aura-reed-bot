...
export default {
  name: ["grabboobs", "agarrartetas", "agarrarpechos"],
  category: "nsfw",
  description: "Agarrar los pechos a un usuario",
  async execute(sock, m, args, { prefix, jidRemitente }) {
    ...
    try {
      const gifData = await getNsfwReactionGif("grabboobs");
      ...
      if (targetJid && targetJid !== jidRemitente) {
        ...
        caption =
          senderTag +
          " " +
          fytBold("le agarró los pechos a") +
          " @" +
          targetJid.split("@")[0] +
          " 🔞";
      } else {
        caption = senderTag + " " + fytBold("se agarra los pechos 🔞");
      }
