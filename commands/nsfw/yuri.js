...
export default {
  name: ["yuri", "lesbian"],
  category: "nsfw",
  description: "Momento yuri con un usuario",
  async execute(sock, m, args, { prefix, jidRemitente }) {
    ...
    try {
      const gifData = await getNsfwReactionGif("yuri");
      ...
      if (targetJid && targetJid !== jidRemitente) {
        ...
        caption =
          senderTag +
          " " +
          fytBold("tuvo un momento yuri con") +
          " @" +
          targetJid.split("@")[0];
      } else {
        caption = senderTag + " " + fytBold("está en modo yuri 🔞");
      }
