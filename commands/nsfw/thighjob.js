...
export default {
  name: ["thighjob", "pajamuslos"],
  category: "nsfw",
  description: "Hacer paja con los muslos a un usuario",
  async execute(sock, m, args, { prefix, jidRemitente }) {
    ...
    try {
      const gifData = await getNsfwReactionGif("thighjob");
      ...
      if (targetJid && targetJid !== jidRemitente) {
        ...
        caption =
          senderTag +
          " " +
          fytBold("le hizo un thighjob a") +
          " @" +
          targetJid.split("@")[0] +
          " 🔞";
      } else {
        caption = senderTag + " " + fytBold("hace un thighjob 🔞");
      }
