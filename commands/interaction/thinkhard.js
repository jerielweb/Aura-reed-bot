...
export default {
    ...
    description: 'Envía una reacción "thinkhard" en formato GIF.',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            const { image, mimetype } = await getReactionGif('thinkhard');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('piensa intensamente sobre') + ' ' + targetTag;
            } else {
                caption = senderTag + ' ' + fytBold('está pensando intensamente 🧠');
            }
