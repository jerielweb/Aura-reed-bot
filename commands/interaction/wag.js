...
export default {
    name: ['wag', 'movercola', 'colita', 'mover', 'menearcola'],
    category: 'interaction',
    description: 'Mover la Cola',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            ...
            const gifData = await getReactionGif('wag');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('le mueve la colita a') + ' @' + targetJid.split('@')[0];
            } else {
