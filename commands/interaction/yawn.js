...
export default {
    ...
    description: 'Envía una reacción "yawn" en formato GIF animado.',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            ...
            const gifData = await getReactionGif('yawn');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('bosteza frente a') + ' @' + targetJid.split('@')[0];
            } else {
                caption = senderTag + ' ' + fytBold('está bostezando 🥱');
            }
