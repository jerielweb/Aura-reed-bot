...
export default {
    name: ['blush', 'sonrojarse', 'sonrojado', 'sonrojada', 'apenado', 'apenada', 'sonrojo'],
    ...
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            ...
            const gifData = await getReactionGif('blush');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('se sonrojó por') + ' @' + targetJid.split('@')[0];
            } else {
                caption = senderTag + ' ' + fytBold('se sonrojó 😳');
            }
