...
export default {
    name: ['wave', 'saludar', 'hola', 'hols', 'hi', 'hello', 'heloo', 'holas', 'helow', 'helowww', 'holis', 'halu', 'hallow', 'holahola', 'holitas', 'holita', 'helo', 'ola'],
    category: 'interaction',
    description: 'Mandar Saludos',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            ...
            const gifData = await getReactionGif('wave');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('saluda con la mano a') + ' @' + targetJid.split('@')[0];
            } else {
                caption = senderTag + ' ' + fytBold('saluda con la mano 👋');
            }
