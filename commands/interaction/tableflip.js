...
export default {
    name: ['tableflip', 'voltearmesa', 'tirarllave', 'voltear', 'tirarmesa'],
    category: 'interaction',
    description: 'Voltear una Mesa',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            ...
            const gifData = await getReactionGif('tableflip');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('voltea la mesa ante') + ' @' + targetJid.split('@')[0];
            } else {
                caption = senderTag + ' ' + fytBold('voltea la mesa (╯°□°)╯︵ ┻━┻');
            }
