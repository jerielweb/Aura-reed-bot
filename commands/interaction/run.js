...
export default {
    ...
    description: 'Envía una reacción "run" en formato GIF animado.',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            ...
            const gifData = await getReactionGif('run');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('corre de') + ' @' + targetJid.split('@')[0];
            } else {
                caption = senderTag + ' ' + fytBold('está corriendo 🏃');
            }
