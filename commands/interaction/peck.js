...
export default {
    name: ['peck', 'besito', 'picotazo', 'beso', 'pico'],
    category: 'interaction',
    description: 'Dar un Besito',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            ...
            const gifData = await getReactionGif('peck');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('le dio un besito a') + ' @' + targetJid.split('@')[0];
            } else {
                caption = senderTag + ' ' + fytBold('tiró un besito 😚');
            }
