...
export default {
    name: ['call', 'llamar', 'llamada', 'telefono', 'celular', 'marcar'],
    category: 'interaction',
    description: 'Llamar por Teléfono',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            ...
            const gifData = await getReactionGif('call');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('está llamando a') + ' @' + targetJid.split('@')[0];
            } else {
                caption = senderTag + ' ' + fytBold('está haciendo una llamada 📞');
            }
