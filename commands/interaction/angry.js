...
export default {
    name: ['angry', 'enojado', 'enojada', 'enfadado', 'enfadada'],
    category: 'interaction',
    description: 'Envía una reacción "angry" en formato GIF.',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            const { image, mimetype } = await getReactionGif('angry');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('está enojado/a con') + ' ' + targetTag;
            } else {
