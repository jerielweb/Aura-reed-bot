...
export default {
    name: ['handshake', 'apreton', 'saludomano', 'estrecharmano', 'apretóndemano'],
    category: 'interaction',
    description: 'Estrechar Manos',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            ...
            const gifData = await getReactionGif('handshake');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('le da la mano a') + ' @' + targetJid.split('@')[0];
            } else {
                caption = senderTag + ' ' + fytBold('saluda a todos 🤝');
            }
