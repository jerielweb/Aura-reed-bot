...
export default {
    name: ['carry', 'cargar', 'alzar', 'llevar', 'alza', 'cargando'],
    category: 'interaction',
    description: 'Cargar a Alguien',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            ...
            const gifData = await getReactionGif('carry');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('está cargando a') + ' @' + targetJid.split('@')[0];
            } else {
                caption = senderTag + ' ' + fytBold('quiere que lo/la carguen 🫂');
            }
