...
export default {
    ...
    description: 'Envía una reacción "tickle" en formato GIF.',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            const { image, mimetype } = await getReactionGif('tickle');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('le hace cosquillas a') + ' ' + targetTag;
            } else {
                caption = senderTag + ' ' + fytBold('quiere hacer cosquillas 😂');
            }
