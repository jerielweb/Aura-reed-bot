...
export default {
    name: ['feed', 'alimentar', 'darcomer', 'nutrir', 'comida', 'alimentando'],
    category: 'interaction',
    description: 'Alimentar a Alguien',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            ...
            const gifData = await getReactionGif('feed');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('está alimentando a') + ' @' + targetJid.split('@')[0];
            } else {
                caption = senderTag + ' ' + fytBold('está comiendo 🍕');
            }
