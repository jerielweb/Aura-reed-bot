...
export default {
    ...
    description: 'Envía una reacción "cringe" en formato GIF.',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            const { image, mimetype } = await getReactionGif('cringe');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('siente cringe por') + ' ' + targetTag;
            } else {
                caption = senderTag + ' ' + fytBold('sintió cringe 😬');
            }
