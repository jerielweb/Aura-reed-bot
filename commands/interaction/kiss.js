...
export default {
    ...
    description: 'Envía una reacción "kiss" en formato GIF.',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            const { image, mimetype } = await getReactionGif('kiss');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('besó a') + ' ' + targetTag;
            } else {
                caption = senderTag + ' ' + fytBold('quiere un beso 💋');
            }
