...
export default {
    name: ['yeet', 'lanzar', 'tirar', 'arrojar', 'yeeteando'],
    category: 'interaction',
    description: 'Lanzar (Yeet)',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            ...
            const gifData = await getReactionGif('yeet');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('lanzó a') + ' @' + targetJid.split('@')[0];
            } else {
                caption = senderTag + ' ' + fytBold('se lanzó al vacío ☄️');
            }
