...
export default {
    name: ['kisscheek', 'besomejilla', 'besocachete', 'cachete', 'mejilla'],
    category: 'interaction',
    description: 'Besar en la Mejilla',
    async execute(sock, m, args, { prefix, jidRemitente }) {
        ...
        try {
            ...
            const gifData = await getReactionGif('kisscheek');
            ...
            if (targetJid && targetJid !== jidRemitente) {
                ...
                caption = senderTag + ' ' + fytBold('le dio un beso en la mejilla a') + ' @' + targetJid.split('@')[0];
            } else {
                caption = senderTag + ' ' + fytBold('mandó un beso en la mejilla 😘');
            }
