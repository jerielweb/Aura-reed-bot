...
function decodeStickerResponse(data) {
  ...
  if (Buffer.isBuffer(data)) {
    ...
    if (payload) {
      const raw = typeof payload === 'string' && payload.startsWith('data:')
        ? payload.split(',')[1]
        : payload
      return Buffer.from(raw, 'base64')
    }
  ...
  if (text.startsWith('{') || text.startsWith('[')) {
    try {
      ...
      const payload = parsed.image || parsed.result || parsed.data || parsed.sticker
      if (payload) {
        const raw = typeof payload === 'string' && payload.startsWith('data:')
          ? payload.split(',')[1]
          : payload
...
async function raceStickerApis(apis) {
  const runners = apis.map(api => api.request().then(res => {
  ...
  throw new Error(results.map(r => r.status === 'rejected' ? r.reason?.message || String(r.reason) : 'ok').join(', '))
}
    ...
    const apis = [
      {
        ...
        request: () => axios.get(buildUrl(global.Apis.apiAiya.url, 'tools/brat'), {
      ...
      },
      {
        ...
        request: () => axios.get(buildUrl(global.Apis.appiFaa.url, 'faa/brat'), {
          params: { text },
          responseType: 'arraybuffer',
          timeout: 60000
        })
      },
      {
        ...
        request: () => axios.get(buildUrl(global.Apis.apiAiya.url, 'api/v1/utilidades/brat'), {
